 ...
 *
 * @RestResource(
 *   id = "concierto_cs_delete_contact",
 *   label = @Translation("concierto cs delete contact"),
 *  serialization_class = "",
 *   uri_paths = {
 *      "canonical" = "/api/csDeleteContact/{id}",
 *   }
 * )
 */
class csDeleteContact extends ResourceBase {
    ...
    /** @var \Drupal\contentservice\Service\GenericService $service */
		...
		$has_content_permission = $service->UserPermissionCheck('Contact','delete');
		if($has_content_permission=='Allow') {
		  ...
		  if (!empty($node)) {
            ...
            $node->delete();
		    $result['response'] = ['status' => 'success', 'message' => 'Contact '.$id.' is deleted Successfully'];
          }
		  else {
            $result['response'] = ['status' => 'failure', 'message' => 'Invalid Contact '.$id];
		  }
