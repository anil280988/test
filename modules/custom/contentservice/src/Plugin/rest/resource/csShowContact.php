...
class csShowContact extends ResourceBase {
    ...
    public function get() {
		...
		if($burl == 'us-east-userapi.concierto.cloud' || $burl == 'uat-userapi.concierto.cloud') {
			$entities = $entities->sort('field_region_order', 'ASC');
		}else{
			$entities = $entities->sort('created', 'DESC');
		}
			$entities = $entities->accessCheck(false)   
					->execute();
		...
		if(!empty($entities)){
			...
			foreach ($nodes as $key => $entity) {
				$result[] = [
					...
					"field_contact_email" => $entity->field_contact_email->value,
					"field_contact_title" => $entity->field_contact_title->value,
					"field_mobile" =>$entity->field_mobile->value,
					"field_region" => $entity->field_region->value,
					"field_profile_image_url" => $entity->field_profile_image_url->value,
					];
			}
