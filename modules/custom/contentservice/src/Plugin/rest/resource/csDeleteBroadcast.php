 ...
 *
 * @RestResource(
 *   id = "concierto_cs_delete_broadcast",
 *   label = @Translation("concierto cs delete broadcast"),
 *  serialization_class = "",
 *   uri_paths = {
 *      "canonical" = "/api/csDeleteBroadcast/{id}",
 *   }
 * )
 */
class csDeleteBroadcast extends ResourceBase {
	...
	/** @var \Drupal\contentservice\Service\GenericService $service */
		...
		$has_content_permission = $service->UserPermissionCheck('Broadcast','delete');
		if($has_content_permission=='Allow') {	
			...
			if (!empty($node)) {
				...
				$node->delete();
				$result = 'Broadcast '.$id.' is deleted Successfully';
            }
			else {
               $result = 'Broadcast '.$id.' is failure to delete';
			}
